...
argparser.add_argument(
    ...
    default="setting_benchmark_ibm_hub_simulator_qcbm_ch42_16_qubits_v10.json",
)
argparser.add_argument(
...
experiment = LegacyExperiment(run_id=f"benchmark-lstm-{prior_model}-{run_date_time}",root_dir= Path(experiment_root).resolve())
...
prior_train = True
noisy_sim_backend = QulacsSimulator()
...
if prior_model == "QCBM":
    ...
    # multiqcbm_ansatz = EntanglingLayerAnsatz(n_qubits=bitstring_size//2, n_layers=n_layers, entangling_layer_builder=entangling_layer_builder)
    # # We create a circuit that rotates the basis of the qubits at the end of the circuit
    # rotate_basis_circuit = RotationLayer(n_qubits=bitstring_size//2)
    # multiqcbm = MultiBasisWavefunctionQCBM(ansatz=multiqcbm_ansatz, optimizer=optimizer, backend=QulacsSimulator(), choices=(0, 1), use_efficient_training=False, basis_rotations=rotate_basis_circuit)
    ...
    prior_train = False
elif prior_model == "ibm_hub_simulator":
    # backend_name = "ibmq_qasm_simulator"
    provider = IBMQ.get_provider(hub="ibm-q")
    if prior_sample_size == 16:
        device_noise = FakeGuadalupe()
    elif prior_sample_size == 27:
        device_noise = FakeHanoi()
    noise_model = NoiseModel.from_backend(device_noise)
    ...
    circuit = export_to_qiskit(qcbm_ansatz.parametrized_circuit)
...
prior_samples_current = torch.tensor(prior.generate(5000)).float()
encoded_compounds = model.generate(prior_samples_current)

smiles_benchamrk = selfies.decode_fn(encoded_compounds)
data = {
    "smiles":encoded_compounds
}
figure_path = epoch_plot_dir / f"smiles_benchamrk.csv"
