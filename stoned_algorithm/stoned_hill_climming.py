...
# df = pd.read_csv("/home/mghazi/workspace/insilico-drug-discovery/benchmark_models/genereted_samples/ligands_1SYH_qcbm_sim.csv")
# smiles_all = df.query("filter_value == True").smiles.to_list()[0:1000]
    ...
    for i,item in enumerate(smiles_all):
        ...
        if pass_filter == True: 
            ...
            for smi_check in all_smiles_collect: 
                ...
                if apply_filters(smi_check) and smi_check not in pass_all and passes_wehi_mcf(smi_check) and (len(pains_filt(Chem.MolFromSmiles(smi_check))) == 0) :
...
pass_all = [x+'\n' for x in pass_all]
with open('./output_stoned_sim_0.8.txt', 'a+') as f: 
